  ...
  render() {
    ...
    const card = (
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-card">
        ...
        {detail && (
          <pre className="mt-4 max-h-40 overflow-auto rounded-lg bg-slate-50 p-3 text-left font-mono text-[11px] leading-5 text-red-600">
            {detail}
          </pre>
        )}
      </div>
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-100 p-6">{card}</div>
    );
  }
}
