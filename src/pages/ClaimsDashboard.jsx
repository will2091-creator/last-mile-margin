...
import { Check, ChevronLeft, ChevronRight, Mail, Paperclip } from "lucide-react";
import EmptyState, { InlineEmpty } from "../components/EmptyState";
import { fileToCompressedImage } from "../lib/imagePrep";
import { uploadClaimEvidence, loadClaimEvidence } from "../lib/evidenceRepository";
...
function ClaimsDashboard({ claims, setClaims, teams, isDark, appSettings, backendStatus, navigateToTab }) {
  ...
  // Evidence files attached to the open claim, keyed by checklist label.
  const [attachedEvidence, setAttachedEvidence] = useState({}); // { [label]: { name, file_path } }
  const [uploadingEvidence, setUploadingEvidence] = useState(null); // label currently uploading
  const [evidenceError, setEvidenceError] = useState("");
  const evidenceInputRef = useRef(null);
  const evidenceTargetLabel = useRef(null);
  ...
  // Build a clean, print-ready dispute packet (claim summary + cited evidence +
  // the letter) and open the browser print dialog so the user can save it as a
  // PDF — no server-side document generation required. Falls back to an HTML
  // download if a popup is blocked.
  const exportDisputePacketPdf = () => {
    const claim = disputePacketClaim;
    if (!claim) return;
    const esc = (value) =>
      String(value ?? "")
    ...
    const summaryRows = [
      ["Claim ID", claim.id],
      ["Claim amount", currency.format(Number(claim.amount || 0))],
      ["Route / date", `${claim.route || "No route"} · ${claim.date || "No date"}`],
      ["Team / driver", `${getClaimTeam(claim)} · ${getClaimDriver(claim) || "Unassigned"}`],
      ["Claim type", claim.type],
      ["Preventability", claim.preventable || "Unknown"],
    ];
    const evidence = getEvidenceChecklist(claim).filter((item) => selectedEvidence.has(item.label));
    const letterHtml = esc(disputeLetterBody || "Generate the dispute letter before exporting to include it here.")
      .replace(/\n/g, "<br/>");

    const doc = `<!doctype html><html><head><meta charset="utf-8"/>
<title>Dispute Packet — ${esc(claim.id)}</title>
<style>
  * { box-sizing: border-box; }
  ...
  h2 { font-size: 14px; text-transform: uppercase; letter-spacing: 0.05em; color: #475569; margin: 28px 0 10px; border-bottom: 1px solid #e2e8f0; padding-bottom: 6px; }
  .muted { color: #64748b; font-size: 12px; }
  table { width: 100%; border-collapse: collapse; font-size: 13px; }
  td { padding: 7px 10px; border: 1px solid #e2e8f0; vertical-align: top; }
  td.label { width: 38%; color: #475569; font-weight: 600; background: #f8fafc; }
  ul { margin: 0; padding-left: 18px; font-size: 13px; }
  .letter { white-space: normal; font-size: 13px; border: 1px solid #e2e8f0; border-radius: 8px; padding: 16px; background: #f8fafc; }
  .foot { margin-top: 32px; color: #94a3b8; font-size: 11px; }
  @media print { body { margin: 18mm; } }
</style></head><body>
  <h1>Dispute Packet</h1>
  <p class="muted">${esc(companyName)} · ${esc(claim.id)} · Prepared ${esc(today)}</p>
  <h2>Claim Summary</h2>
  <table><tbody>
    ${summaryRows.map(([l, v]) => `<tr><td class="label">${esc(l)}</td><td>${esc(v)}</td></tr>`).join("")}
  </tbody></table>
  <h2>Evidence Cited</h2>
  ${evidence.length
    ? `<ul>${evidence.map((item) => `<li>${esc(item.label)}${item.present ? "" : " (to be attached)"}</li>`).join("")}</ul>`
    : `<p class="muted">No evidence items selected.</p>`}
  <h2>Dispute Letter</h2>
  <div class="letter">${letterHtml}</div>
  <p class="foot">Generated by Last Mile Margin.</p>
</body></html>`;
    ...
    // Popup blocked — download the packet as HTML the user can open and print.
    const blob = new Blob([doc], { type: "text/html" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `dispute-packet-${claim.id}.html`;
  ...
  // Load any previously-attached evidence files when a dispute packet opens.
  useEffect(() => {
    if (!disputePacketClaim) { setAttachedEvidence({}); setEvidenceError(""); return; }
    let cancelled = false;
    setEvidenceError("");
    loadClaimEvidence(disputePacketClaim.id).then((result) => {
      if (cancelled || !result.ok) return;
      const map = {};
      for (const row of result.evidence) {
        const key = row.notes || row.name;
        if (key && !map[key]) map[key] = { name: row.name, file_path: row.file_path };
      }
      setAttachedEvidence(map);
    });
    return () => { cancelled = true; };
  }, [disputePacketClaim]);

  const openEvidencePicker = (label) => {
    evidenceTargetLabel.current = label;
    evidenceInputRef.current?.click();
  };

  const handleEvidenceFileSelected = async (event) => {
    const file = event.target.files?.[0];
    event.target.value = ""; // allow re-selecting the same file
    const label = evidenceTargetLabel.current;
    if (!file || !label || !disputePacketClaim) return;
    setEvidenceError("");
    setUploadingEvidence(label);
    const result = await uploadClaimEvidence({ file, appClaimId: disputePacketClaim.id, label });
    setUploadingEvidence(null);
    if (!result.ok) {
      setEvidenceError(result.error || "Couldn't upload that file. Try again.");
      return;
    }
    setAttachedEvidence((current) => ({ ...current, [label]: { name: file.name, file_path: result.evidence?.file_path } }));
    // An attached file satisfies the checklist item — make sure it's cited.
    setSelectedEvidence((current) => new Set(current).add(label));
  };
          ...
          <div className={isDark ? "max-h-[90vh] w-full max-w-5xl overflow-hidden rounded-3xl border border-white/10 bg-slate-950 shadow-2xl shadow-black/50" : "max-h-[90vh] w-full max-w-5xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-950/20"}>
            <div className={isDark ? "flex flex-col gap-4 border-b border-white/10 p-5 lg:flex-row lg:items-start lg:justify-between" : "flex flex-col gap-4 border-b border-slate-200 p-5 lg:flex-row lg:items-start lg:justify-between"}>
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={exportDisputePacketPdf}
                  title="Open a print-ready packet — choose 'Save as PDF' in the print dialog."
                  className={isDark ? "rounded-xl border border-white/10 px-4 py-2 text-sm font-black text-white hover:bg-white/10" : "rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-black text-slate-700 hover:bg-slate-50"}
                >
                  Export PDF
                </button>
              ...
              <div className="grid gap-5 lg:grid-cols-[1fr_1.1fr]">
                <div className="space-y-5">
                  ...
                  <div className={isDark ? "rounded-2xl border border-white/10 bg-slate-900/80 p-5" : "rounded-2xl border border-slate-200 bg-slate-50 p-5"}>
                    ...
                    <p className={`mt-1 text-xs font-semibold ${mutedText}`}>AI pre-selected what to cite. Check or uncheck to control what goes in the letter — and attach the actual file for each item.</p>
                    <input
                    ...
                    />
                    {evidenceError && (
                      <p className="mt-3 rounded-lg bg-red-500/10 px-3 py-2 text-xs font-bold text-red-600">{evidenceError}</p>
                    )}
                    <div className="mt-4 space-y-2">
                      {getEvidenceChecklist(disputePacketClaim).map((item) => {
                        const checked = selectedEvidence.has(item.label);
                        const attached = attachedEvidence[item.label];
                        const uploading = uploadingEvidence === item.label;
                        return (
                          <div
                            key={item.label}
                            className={`flex items-center justify-between gap-3 rounded-xl border p-3 transition ${
                              isDark
                                ? checked ? "border-blue-500/40 bg-blue-500/10" : "border-white/10 bg-slate-950/60"
                          ...
                          >
                            ...
                            >
                              ...
                              <span className="min-w-0">
                                <span className={`block text-sm font-bold ${titleText}`}>{item.label}</span>
                                {attached && (
                                  <span className={`block truncate text-[11px] font-semibold ${mutedText}`}>{attached.name}</span>
                                )}
                              </span>
                            </button>
                            <div className="flex shrink-0 items-center gap-2">
                              <span className={(attached || item.present) ? "rounded-full bg-emerald-500/10 px-2.5 py-1 text-xs font-black text-emerald-700" : "rounded-full bg-red-500/10 px-2.5 py-1 text-xs font-black text-red-600"}>
                                {attached ? "Attached" : item.present ? "Ready" : "Missing"}
                              </span>
                              <button
                                type="button"
                                onClick={() => openEvidencePicker(item.label)}
                                disabled={uploading}
                                title={attached ? "Replace file" : "Attach file"}
                                className={isDark ? "flex h-8 w-8 items-center justify-center rounded-lg border border-white/10 text-slate-300 hover:bg-white/10 disabled:opacity-50" : "flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-100 disabled:opacity-50"}
                              >
                                {uploading
