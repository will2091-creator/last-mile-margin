...
function DashboardHome({ teams, claims, setActiveTab, isDark, appSettings, savedDaySnapshot }) {
  const widgets = appSettings?.dashboardWidgets || defaultSettings.dashboardWidgets;
  ...
  const selectedWidgetCount = Object.values(widgets).filter(Boolean).length;
  ...
  const dashboardPeriodCards = [
    ...
    ["Revenue", currency.format(dashboardRevenue), "text-slate-900", "Profitability", "Route revenue inputs"],
  ...
  return (
    <div className={pageClass}>
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h1 className={`text-3xl font-black tracking-tight sm:text-4xl ${titleText}`}>Dashboard</h1>
          <p className={`mt-2 text-sm sm:text-base ${mutedText}`}>
            Overview of your routes, profitability, and performance.
          </p>
          {savedDaySnapshot && (
            <p className={isDark ? "mt-3 inline-flex rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-xs font-black text-emerald-200" : "mt-3 inline-flex rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700"}>
              Viewing saved day: {savedDaySnapshot.label}
            </p>
          )}
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-3 xl:grid-cols-6">
        {dashboardPeriodCards.map(([label, value, tone, tab, source]) => (
          <button
            ...
            className={`${isDark ? "rounded-2xl border border-white/10 bg-slate-900/80 p-4 hover:border-blue-500/50 hover:bg-slate-900" : "rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm hover:border-blue-300 hover:bg-blue-50/40"} text-left transition`}
          >
            <p className={`text-xs font-black uppercase tracking-wide ${mutedText}`}>{dashboardPeriod} {label}</p>
            <p className={`mt-2 text-2xl font-black ${tone}`}>{value}</p>
            <p className={`mt-2 text-[11px] font-bold ${mutedText}`}>{source}</p>
          </button>
        ))}
      </div>
            ...
            <button onClick={() => goToSource("Profitability")} className={`${heroClass} block w-full text-left transition hover:border-blue-500`}>
              <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  ...
                  <p className={`mt-6 text-6xl font-black tracking-tight ${titleText}`}>{currency.format(todayProfit)}</p>
                  ...
                  <div className={`mt-6 grid max-w-md grid-cols-2 gap-4 border-t pt-5 ${rowBorder}`}>
                    <div>
                      <p className={`text-3xl font-black ${titleText}`}>{number.format(margin)}%</p>
                      <p className={`text-sm ${mutedText}`}>Margin</p>
                    </div>
                    <div>
                      <p className="text-2xl font-black text-emerald-700">↑ {number.format(profitChange)}%</p>
          ...
          {widgets.financialSummary && (
            <div className="grid gap-4 md:grid-cols-4">
              {[
                ...
                <button key={label} onClick={() => goToSource(tab)} className={`${cardClass} text-left transition hover:border-blue-500/50`}>
                  ...
                  <p className={`mt-2 text-2xl font-black ${titleText}`}>{value}</p>
                  <p className={tone === "amber" ? "mt-4 text-sm font-bold text-amber-700" : "mt-4 text-sm font-bold text-emerald-700"}>
