// Cash Position — the pure calculation "brain" (mobile port).
//
// Ported verbatim from the web app's src/lib/cashPosition.js so the two products
// show identical totals, aging buckets, and the early-pay PREVIEW. It mirrors,
// 1:1, the SQL in docs/supabase-cash-position.sql (get_cash_position_summary /
// get_early_pay_preview), and runs client-side so the screen renders correct
// numbers even when Supabase is not configured / the schema isn't deployed yet.
// Keep this in sync with the web file and the SQL.
//
// MONEY RULE: everything is integer cents (bigint in the DB). We never do float
// dollar math; we only divide by 100 at the formatting boundary in the screen.
